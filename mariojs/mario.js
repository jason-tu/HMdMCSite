...
*/
		...
		init: function() {
			...
			$.getJSON( "MarioJS/objects.json", function( json ) {
				//console.log("OBJECTS",json);
				$.each( objectsJSON, function( k, v ) {
					//console.log(v);
					var x,
						ig = v.ignore ? true : false;
					switch ( v.type ) {
						case "brick":
							x = new brick( 'MarioJS/objects.png', v.sx, v.sy, v.sw, v.sh, v.dx, v.dy, v.dw, v.dh, ig );
							break;
						case "questionmark":
							var r = v.reward ? v.reward : "coin";
							x = new questionmark( 'MarioJS/objects.png', v.sx, v.sy, v.sw, v.sh, v.dx, v.dy, v.dw, v.dh, ig, r );
							break;
						default:
							x = new gameObject( 'MarioJS/objects.png', v.sx, v.sy, v.sw, v.sh, v.dx, v.dy, v.dw, v.dh, ig );
							break;
					}
					if ( x ) z.objects.push( x );
				} );
			} );
			$.getJSON( "MarioJS/enemies.json", function( json ) {
				$.each( enemiesJSON, function( k, v ) {
		...
		moveRight: function() {
			...
			if ( z.get( 'direction' ) == z.LEFT && !z.get( 'isJumping' ) ) {
				...
				if ( z.get( 'currentDX' ) < z.MAXRIGHT && !level.collisions[ 1 ] ) { //move the sprite right
					z.set( {
						currentDX: z.get( 'currentDX' ) + z.XVELOCITY,
						bkgdPos: z.get( 'bkgdPos' ) + z.XVELOCITY
					} );
		...
		jump: function() {
			...
			if ( z.get( 'jumpDirection' ) === "UP" ) {
				if ( curH < z.MAXTOP && !level.collisions[ 0 ] ) {
					z.set( {
						currentDY: curDY - z.YVELOCITY,
						currentHeight: curH + z.YVELOCITY
					} );
				} else {
					z.set( {
						jumpDirection: "DOWN"
